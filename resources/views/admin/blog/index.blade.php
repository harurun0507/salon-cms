...
@section('save-bar')
    <div class="flex min-w-0 flex-wrap items-center justify-between gap-3">
        ...
        <x-admin.create-button data-blog-add-top class="shrink-0">
            ブログを追加
...
@section('content')
    @php
        ...
        foreach ($blogs as $blog) {
            if (in_array((string) $blog->id, $deletedIds, true)) {
                continue;
            }

            $workspaceCards[] = [
                'kind' => 'existing',
                'order' => (int) old('blogs.'.$blog->id.'.display_order', $blog->display_order),
        ...
        foreach ($oldNewBlogs as $key => $newItem) {
            ...
            $workspaceCards[] = [
                'kind' => 'new',
                'order' => (int) old('new_blogs.'.$key.'.display_order', $newItem['display_order'] ?? PHP_INT_MAX),
                'index' => $workspaceIndex++,
                'key' => (string) $key,
                'newItem' => $newItem,
            ];
        ...
        <div id="blog-grid" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3" data-blog-grid>
            @foreach($workspaceCards as $workspaceCard)
                @if($workspaceCard['kind'] === 'existing')
                    @include('admin.blog.partials.card-existing', [
                        'blog' => $workspaceCard['blog'],
                        'formatLocal' => $formatLocal,
                    ])
                @else
                    @include('admin.blog.partials.card-new', [
    ...
    <script>
        (function () {
            ...
            const addTopButton = document.querySelector('[data-blog-add-top]');
            ...
            function createEmptyCard(placement) {
                ...
                if (window.AdminWorkspaceCards && typeof window.AdminWorkspaceCards.insert === 'function') {
                    ...
                    const firstCard = grid.querySelector('[data-blog-card]');
                    if (firstCard) {
