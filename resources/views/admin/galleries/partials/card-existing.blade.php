@php
    ...
    $deletedImageIds = collect(old($prefix.'.deleted_image_ids', []))
    ...
    foreach ($gallery->images as $image) {
        if (in_array((string) $image->id, $deletedImageIds, true)) {
            continue;
        }

        $imgPrefix = $prefix.'.images.'.$image->id;
        $pendingPath = (string) old($imgPrefix.'.pending_image_path', '');
        $pendingUsable = $isUsablePending($pendingPath);
        $previewPath = $pendingUsable ? $pendingPath : $image->image_path;

        $imageSlots[] = [
            'kind' => 'existing',
            'existingId' => $image->id,
            'imageKey' => null,
            'nameBase' => 'galleries['.$gallery->id.'][images]['.$image->id.']',
            'displayOrder' => (int) old($imgPrefix.'.display_order', $image->display_order),
            'altText' => (string) old($imgPrefix.'.alt_text', $image->alt_text),
            'pendingPath' => $pendingUsable ? $pendingPath : '',
            'previewPath' => $previewPath,
            'index' => $slotIndex++,
        ];
    }

    $oldNewImages = old($prefix.'.new_images', []);
    ...
    foreach ($oldNewImages as $imageKey => $newImage) {
        ...
        $imageSlots[] = [
            ...
            'nameBase' => 'galleries['.$gallery->id.'][new_images]['.$imageKey.']',
...
>
    ...
    <div data-gallery-deleted-images>
        @foreach($deletedImageIds as $deletedImageId)
            <input type="hidden" name="galleries[{{ $gallery->id }}][deleted_image_ids][]" value="{{ $deletedImageId }}">
        @endforeach
    </div>
